...
function LoginPage() {
	...
	const onSubmitHandler = (event) => {
		...
		dispatch(loginUser(body)).then((response) => {
			...
			if (response.payload.loginSuccess) {//로그인 성공시 메인페이지 이동
				navigate('/');
			} else {
				if (!response.payload.registerSuccess) { // 로그인은 실패했지만, 이메일 인증 혹은 관리자 승인이 필요
					alert(response.payload.msg);
					if (response.payload?.email) { //이메일 인증이 안된 경우
						dataToSubmit.email = response.payload.email;
						Axios.post('https://edu-trans.ewha.ac.kr:8443/email', dataToSubmit, { //이메일 인증코드 전송
