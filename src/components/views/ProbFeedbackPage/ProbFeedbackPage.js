...
function ProbFeedbackPage() {
  ...
  const onClickButton = () => {
    ...
    console.log(data);
    Axios.get(
      `${API_URL}api/feedback/studentgraphlist?as_no=${data.asnum}&lecture_no=${data.num}&user_no=${data.user_no}`,
      {
        withCredentials: true,
      }
    )
  ...
  useEffect(() => {
    ...
    if (Datacontent !== "") {
      ...
      Axios.post(`${API_URL}r_feedback`, body, {
        withCredentials: true,
      })
        .then((response) => {
  ...
  return (
    <div>
      ...
      <LectureCreateDiv>
        <LectureCreateButton onClick={onClickButton}>
          그래프 보기
        </LectureCreateButton>
