...
import MusicPlayLoading from '../LoadingPage/MusicPlayLoading';
...
function AudioRecordPage() {
	...
	useEffect(() => {
		Axios.get(
			`${API_URL}api/prob/submit?lecture_no=${data.num}&as_no=${data.asnum}`,
			{
			...
			.then((response) => {
				...
				let Music_URL =
					`${API_URL}` + response.data.as_info.upload_url;
